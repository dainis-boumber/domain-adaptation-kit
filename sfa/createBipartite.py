...
def createMatrix(source, target):
	...
	pivotList = list(C.items())
	pivotList.sort(key=lambda x, y: -1 if x[1] > y[1] else 1, reverse=False)
	...
	srcList = list(srcSpecificFeatures.items())
	srcList.sort(key=lambda x, y: -1 if x[1] > y[1] else 1, reverse=False)
	tgtList = list(tgtSpecificFeatures.items())
	tgtList.sort(key=lambda x, y: -1 if x[1] > y[1] else 1, reverse=False)
