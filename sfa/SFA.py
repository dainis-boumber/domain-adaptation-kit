...
def createMatrix(src, tgt):
    ...
    print( "Source = %s, Target = %s" % (src, target))

    # Get the set of source domain features.
    S = {}
    getVocab(S, "../work/%s/train.positive" % src)
    getVocab(S, "../work/%s/train.negative" % src)
    getVocab(S, "../work/%s/train.unlabeled" % src)
    ...
    T = {}
    getVocab(T, "../work/%s/train.positive" % tgt) # labels not used.
    getVocab(T, "../work/%s/train.negative" % tgt) # labels not used.
    getVocab(T, "../work/%s/train.unlabeled" % tgt)
    ...
    getCounts(V, M, "../work/%s/train.positive" % src)
    print ("%s positive %d" % (source, len(M)))
    getCounts(V, M, "../work/%s/train.negative" % src)
    print ("%s negative %d" % (source, len(M)))
    getCounts(V, M, "../work/%s/train.unlabeled" % src)
    print( "%s unlabeled %d" % (source, len(M)))
    getCounts(V, M, "../work/%s/train.positive" % tgt)
    print( "%s positive %d" % (target, len(M)) )
    getCounts(V, M, "../work/%s/train.negative" % tgt)
    print( "%s negative %d" % (target, len(M)))
    getCounts(V, M, "../work/%s/train.unlabeled" % tgt)
    ...
    pivotList = sorted(C.items(), key=compare)
...
def evaluate_SA(src, tgt, project):
    """
    Report the cross-domain sentiment classification accuracy. 
    """
    gamma = 1.0
    print( "Source Domain", src)
    print ("Target Domain", tgt)
    if project:
        print ("Projection ON", "Gamma = %f" % gamma)
    else:
        print ("Projection OFF")
    # Load the projection matrix.
    M = sp.csr_matrix(sio.loadmat("../work/%s-%s/proj.mat" % (src, tgt))['proj'])
    (nDS, h) = M.shape
    # Load the domain specific features.
    DSfeat = {}
    DSFile = open("../work/%s-%s/DS_list" % (src, target))
    for line in DSFile:
        p = line.strip().split()
        DSfeat[p[1].strip()] = int(p[0])
    DSFile.close()
    # write train feature vectors.
    trainFileName = "../work/%s-%s/trainVects.SFA" % (src, tgt)
    testFileName = "../work/%s-%s/testVects.SFA" % (src, tgt)
    featFile = open(trainFileName, 'w')
    count = 0
    for (label, fname) in [(1, 'train.positive'), (-1, 'train.negative')]:
        F = open("../work/%s/%s" % (src, fname))
        for line in F:
            count += 1
            #print "Train ", count
    ...
    modelFileName = "../work/%s-%s/model.SFA" % (src, tgt)
